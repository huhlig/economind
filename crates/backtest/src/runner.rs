...
use economind_db::{
    BacktestRunRow, BacktestStorage, BacktestTradeRow, CandleStorage, DataStore, EquityCurvePoint,
...
impl BacktestRunner {
    ...
    pub async fn run(self, store: &DataStore) -> anyhow::Result<BacktestResult> {
        ...
        let config_snapshot_json =
            serde_json::to_string(&config.strategy_config).unwrap_or_default();
...
async fn simulate(
    ...
    if trading_dates.is_empty() {
        return Err(
            "No trading dates found in the backtest window — ensure bars are ingested".into(),
        );
    }
    ...
    for (idx, &date) in trading_dates.iter().enumerate() {
        ...
        for signal in &signals {
            ...
            if let Some(&open_price) = latest_prices.get(sym) {
                ...
                let max_notional = portfolio.cash.min(portfolio_val * config.max_position_pct);
        ...
        BacktestRunner {
            config: BacktestConfig {
                ...
                initial_capital: self
                    .initial_capital
                    .unwrap_or_else(|| Decimal::from(100_000u32)),
                ...
                max_position_pct: self.max_position_pct.unwrap_or_else(|| Decimal::new(5, 2)), // 5%
