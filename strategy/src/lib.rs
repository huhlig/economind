...
pub use self::ensemble::{
    EnsembleRunner, EnsembleRunnerBuilder, OptimisationObjective, OptimisationResult,
    WeightOptimizer, sharpe_from_equity, sortino_from_equity,
};
...
mod tests {
    ...
    impl Identifier for PassAllIdentifier {
        fn name(&self) -> &str { "pass-all" }
        async fn identify(&self, ctx: &StrategyContext) -> Vec<Candidate> {
            ctx.universe.iter().map(|s| Candidate {
                symbol: s.clone(),
                score: 1.0,
    ...
    impl Identifier for RejectAllIdentifier {
        ...
        async fn identify(&self, _ctx: &StrategyContext) -> Vec<Candidate> { vec![] }
    }
    ...
    fn strategy_config_parameters_roundtrip() {
        ...
        assert_eq!(cfg.parameters.get("fast_ema").map(String::as_str), Some("12"));
        assert_eq!(cfg.parameters.get("slow_ema").map(String::as_str), Some("26"));
    }

    #[test]
    fn plugin_spec_fields() {
        let spec = PluginSpec { role: "timer".to_string(), name: "trend-follow".to_string() };
    ...
    async fn pipeline_passthrough_no_timers_emits_all_candidates() {
        ...
        assert_eq!(signals.len(), 2, "all candidates should pass through with no timers");
    }
    ...
    async fn pipeline_reject_all_identifier_emits_nothing() {
        ...
        let ctx = ctx_with_universe(&["AAPL", "GOOG"]);
        let signals = runner.run(&ctx).await;
        assert!(signals.is_empty(), "reject-all identifier should produce no signals");
    }
    ...
    async fn pipeline_averages_two_timer_scores() {
        ...
        let signals = runner.run(&ctx).await;
        assert_eq!(signals.len(), 1);
        assert!((signals[0].timing.score - 0.7).abs() < 1e-9, "score={}", signals[0].timing.score);
    }
    ...
    async fn strategy_stack_name_in_rationale() {
        ...
        assert!(signals[0].timing.rationale.contains("test-stack"),
            "rationale='{}' should contain stack name", signals[0].timing.rationale);
    }
    ...
    async fn voting_runner_unanimous_emits_signal() {
        ...
        let ctx = ctx_with_universe(&["AAPL"]);
        let runner = VotingRunner::new(vec![
            make_stack(0.9, 0.5),
            make_stack(0.8, 0.5),
        ], 0.5);
        let signals = runner.run(&ctx).await;
    ...
    async fn voting_runner_below_quorum_suppresses_signal() {
        ...
        let signals = runner.run(&ctx).await;
        assert!(signals.is_empty(), "below-quorum vote should produce no signal");
    }
    ...
    async fn ensemble_runner_normalises_weights() {
        ...
        let ctx = ctx_with_universe(&["AAPL"]);
        let runner = EnsembleRunner::new(vec![
            (make_stack(0.8, 0.0), 1.0),
            (make_stack(0.6, 0.0), 1.0),
        ], 0.0);
        let total_weight: f64 = runner.stacks.iter().map(|(_, w)| *w).sum();
        assert!((total_weight - 1.0).abs() < 1e-9, "weights should sum to 1.0");
    ...
    async fn ensemble_runner_below_threshold_suppresses_signal() {
        ...
        let ctx = ctx_with_universe(&["AAPL"]);
        let runner = EnsembleRunner::new(vec![
            (make_stack(0.3, 0.0), 1.0),
        ], 0.5);
        let signals = runner.run(&ctx).await;
        assert!(signals.is_empty(), "score 0.3 < threshold 0.5 should produce no signal");
    }
    ...
    fn sharpe_neg_infinity_for_single_data_point() {
        ...
        assert!(s.is_infinite() && s < 0.0, "sharpe should be NEG_INFINITY for insufficient data");
    }
    ...
    fn sortino_neg_infinity_when_no_downside_returns() {
        ...
        assert!(s.is_infinite() && s < 0.0, "no downside returns should give NEG_INFINITY, got {s}");
    }
    ...
    fn sortino_positive_for_mixed_returns() {
        ...
        let s = sortino_from_equity(&equity);
        assert!(s > 0.0, "positive-edge series should have positive sortino, got {s}");
    }
