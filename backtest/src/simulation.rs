...
mod tests {
    ...
    fn exit_position_produces_correct_trade() {
        ...
        assert!(t.gross_pnl > Decimal::ZERO, "gross_pnl should be positive (price up)");
        assert!(t.net_pnl < t.gross_pnl, "net_pnl < gross_pnl after commission");
    }
    ...
    fn exit_position_increases_cash_on_profit() {
        ...
        assert!(cash_after_exit > cash_after_entry, "cash should increase after profitable exit");
        assert!(cash_after_exit > cash_before - d("5"), "net of costs, cash should be near original");
    }
    ...
    fn portfolio_value_includes_open_positions_at_mark() {
        ...
        assert!(val > d("100000"), "portfolio value should exceed capital due to mark-up");
    ...
    fn close_all_closes_every_open_position() {
        ...
        p.close_all(date(2024, 1, 10), &prices);
        assert!(p.open_positions.is_empty(), "all positions should be closed");
