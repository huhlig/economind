...
use crate::{DailyDataProvider, IntradayDataProvider, ProviderResult, RateLimitedClient};
use chrono::{NaiveDate, NaiveDateTime};
use economind_core::model::{CandleEntry, DailyCandleEntry, Interval, Symbol};
use governor::Quota;
    ...
    async fn daily_bars(
        ...
        for record in reader.records() {
            ...
            entries.push(DailyCandleEntry {
                date,
                open: Decimal::from_f32_retain(record[1].parse().unwrap_or(0.0))
                    .unwrap_or_default(),
                high: Decimal::from_f32_retain(record[2].parse().unwrap_or(0.0))
                    .unwrap_or_default(),
                low: Decimal::from_f32_retain(record[3].parse().unwrap_or(0.0)).unwrap_or_default(),
                close: Decimal::from_f32_retain(record[4].parse().unwrap_or(0.0))
                    .unwrap_or_default(),
