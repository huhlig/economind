...
use crate::storage::traits::{CandleStorage, MetadataStorage, TickStorage, TickerQuery};
...
impl MetadataStorage for PostgresStorage {
    ...
    async fn query_tickers<'a>(
        ...
        let tickers: Vec<Ticker> = rows.into_iter().map(row_to_ticker).collect();
...
impl CandleStorage for PostgresStorage {
    ...
    async fn unsert_daily_candle(
        ...
        for bar in bars {
            let ts = Utc.from_utc_datetime(&bar.date.and_hms_opt(0, 0, 0).unwrap());
            sqlx::query(
                "INSERT INTO market.bars (symbol, interval, time, open, high, low, close, volume) \
    ...
    async fn query_daily_candles<'a>(
        ...
        let entries: Vec<DailyCandleEntry> =
            rows.into_iter().map(row_to_daily_candle_entry).collect();
...
fn row_to_news(row: sqlx::postgres::PgRow) -> NewsStory {
    ...
    NewsStory {
        ...
        evaluation: row
            .get::<Option<String>, _>("evaluation")
            .unwrap_or_default(),
        published_at,
        fetched_at,
    }
...
fn row_to_income_statement(row: sqlx::postgres::PgRow) -> IncomeStatement {
    IncomeStatement {
        ...
        operating_income: row
            .get::<Option<Decimal>, _>("operating_income")
            .unwrap_or_default(),
        ebit: row.get::<Option<Decimal>, _>("ebit").unwrap_or_default(),
        net_income: row
            .get::<Option<Decimal>, _>("net_income")
            .unwrap_or_default(),
        eps: row.get::<Option<Decimal>, _>("eps").unwrap_or_default(),
        interest_expense: row
            .get::<Option<Decimal>, _>("interest_expense")
            .unwrap_or_default(),
        tax_expense: row
            .get::<Option<Decimal>, _>("tax_expense")
            .unwrap_or_default(),
    }
}
...
fn row_to_balance_sheet(row: sqlx::postgres::PgRow) -> BalanceSheet {
    BalanceSheet {
        ...
        period_end: row.get("period_end"),
        total_assets: row
            .get::<Option<Decimal>, _>("total_assets")
            .unwrap_or_default(),
        total_debt: row
            .get::<Option<Decimal>, _>("total_debt")
            .unwrap_or_default(),
        total_equity: row
            .get::<Option<Decimal>, _>("total_equity")
            .unwrap_or_default(),
...
fn row_to_dividend(row: sqlx::postgres::PgRow) -> DividendEvent {
    DividendEvent {
        ...
        payment_date: row
            .get::<Option<NaiveDate>, _>("payment_date")
