...
pub async fn load_context(
    ...
    let mut bars: HashMap<Symbol, Vec<DailyCandleEntry>> = HashMap::with_capacity(universe.len());
    for sym in &universe {
    ...
    // 4. Portfolio state.
    let portfolio = store.load_portfolio_state().await.unwrap_or_else(|_| {
        economind_db::storage::PortfolioState {
            open_positions: vec![],
