...
use economind_core::model::{
    ...
    IncomeStatement, Interval, NewsAbout, NewsStory, StockSplitEvent, Symbol, Ticker,
};
...
async fn duckdb_intraday_candle_round_trip() {
    ...
    let t = |h: u32, m: u32| -> NaiveDateTime {
        date(2024, 3, 15).and_hms_opt(h, m, 0).unwrap()
    };

    let bars = vec![
        CandleEntry { timestamp: t(9, 30),  open: dec("505.00"), high: dec("506.00"), low: dec("504.50"), close: dec("505.80"), volume: 1_200_000 },
        CandleEntry { timestamp: t(9, 35),  open: dec("505.80"), high: dec("507.00"), low: dec("505.00"), close: dec("506.50"), volume: 980_000  },
        CandleEntry { timestamp: t(9, 40),  open: dec("506.50"), high: dec("508.00"), low: dec("506.00"), close: dec("507.20"), volume: 870_000  },
    ];

    db.upsert_candles(&sym("SPY"), Interval::FiveMinute, &bars).await.unwrap();

    let result = db.query_candles(
        &sym("SPY"),
        Interval::FiveMinute,
        t(9, 0)..t(10, 0),
    ).await.unwrap();
...
async fn duckdb_income_statement_round_trip() {
    ...
    let result = db.query_income_statements(Some(&sym("NVDA")), None).await.unwrap();
    ...
    assert!(eps_diff < dec("0.01"), "eps round-trip within 1 cent: got {}", result[0].eps);
}
...
async fn duckdb_balance_sheet_round_trip() {
    ...
    let result = db.query_balance_sheets(Some(&sym("AAPL")), None).await.unwrap();
    assert_eq!(result.len(), 1);
...
async fn duckdb_cash_flow_round_trip() {
    ...
    let result = db.query_cash_flow_statements(Some(&sym("MSFT")), None).await.unwrap();
    assert_eq!(result.len(), 1);
...
async fn duckdb_dividend_round_trip() {
    ...
    let items = vec![
        DividendEvent { symbol: sym("JNJ"), ex_date: date(2024, 2, 20), payment_date: date(2024, 3, 5), amount: dec("1.19") },
        DividendEvent { symbol: sym("JNJ"), ex_date: date(2024, 5, 21), payment_date: date(2024, 6, 4), amount: dec("1.24") },
    ];

    db.insert_dividend_report(&items).await.unwrap();

    let all = db.query_dividend_report(Some(&sym("JNJ")), None).await.unwrap();
    assert_eq!(all.len(), 2);

    let range = db.query_dividend_report(
        Some(&sym("JNJ")),
        Some(date(2024, 1, 1)..date(2024, 4, 1)),
    ).await.unwrap();
...
async fn duckdb_stock_split_round_trip() {
    ...
    let result = db.query_stock_split(Some(&sym("NVDA")), None).await.unwrap();
    assert_eq!(result.len(), 1);
...
async fn duckdb_news_round_trip() {
    ...
    let aapl_news: Vec<NewsStory> = {
        use futures::StreamExt;
        db.query_news(Some(&sym("AAPL")), None).await.unwrap().collect().await
    };
...
//
// These require DATABASE_URL to point to a live PostgreSQL + TimescaleDB instance
// with migrations applied.  They are skipped automatically when DATABASE_URL is
// not set, so they never block CI that lacks a database.

#[cfg(test)]
mod postgres_tests {
    use super::*;
    use economind_db::storage::PostgresStorage;

    /// Returns `None` and prints a skip message if DATABASE_URL is not set.
    async fn maybe_connect() -> Option<PostgresStorage> {
        let url = match std::env::var("DATABASE_URL") {
            Ok(u) => u,
            Err(_) => {
                eprintln!("SKIP: DATABASE_URL not set — skipping PostgreSQL smoke tests");
                return None;
            }
        };
        match PostgresStorage::connect(&url).await {
            Ok(db) => Some(db),
            Err(e) => {
                eprintln!("SKIP: could not connect to PostgreSQL ({e}) — skipping");
                None
            }
        }
    }
    ...
    #[tokio::test]
    async fn postgres_connection_and_list_tickers() {
        let Some(db) = maybe_connect().await else { return };

        // list_tickers must succeed even on an empty (freshly migrated) database
        let symbols: Vec<Symbol> = {
            use futures::StreamExt;
            db.list_tickers().await.unwrap().collect().await
        };
        // We don't assert a specific count — just that the query runs
        eprintln!("postgres: found {} active instruments", symbols.len());
    }

    #[tokio::test]
    async fn postgres_instrument_upsert_and_get() {
        let Some(db) = maybe_connect().await else { return };

        let test_sym = sym("__SMOKE_TEST__");

        // Insert
        db.upsert_ticker(&test_sym).await.unwrap();

        // Retrieve
        let ticker = db.get_ticker(&test_sym).await.unwrap();
        assert!(ticker.is_some(), "upserted ticker must be retrievable");
        assert_eq!(ticker.unwrap().symbol, test_sym);
    ...
    #[tokio::test]
    async fn postgres_daily_candle_round_trip() {
        let Some(db) = maybe_connect().await else { return };
        ...
        db.unsert_daily_candle(&test_sym, &bars).await.unwrap();

        let result: Vec<DailyCandleEntry> = {
            use futures::StreamExt;
            db.query_daily_candles(&test_sym, date(2000, 1, 1)..date(2000, 1, 31))
                .await
                .unwrap()
                .collect()
                .await
        ...
        assert_eq!(result.len(), 1);
        assert_eq!(result[0].close, dec("100.50"));
    }
}
