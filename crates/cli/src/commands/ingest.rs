...
async fn run_macro(args: MacroArgs, store: &DataStore) -> anyhow::Result<()> {
    let fred = FredConnector::from_env().map_err(|e| {
        anyhow::anyhow!("Failed to create FRED connector — is FRED_API_KEY set? {e}")
    })?;
