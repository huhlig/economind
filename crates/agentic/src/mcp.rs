...
impl EconomindMcpServer {
    ...
    async fn get_signals(
        ...
        let symbol = p.symbol.as_deref().map(economind_core::model::Symbol::new);

        let config_id = p.config_id.as_deref().and_then(|s| Uuid::parse_str(s).ok());
    ...
    async fn analyze_signal(
        ...
        let brief = crate::analysis::analyze_signal(&self.store, self.llm.as_deref(), signal_id)
            .await
            .map_err(|e| McpError::internal_error(e.to_string(), None))?
            .ok_or_else(|| {
    ...
    async fn analyze_instrument(
        ...
        let brief =
            crate::analysis::analyze_instrument(&self.store, self.llm.as_deref(), &p.symbol)
                .await
                .map_err(|e| McpError::internal_error(e.to_string(), None))?
                .ok_or_else(|| {
                    McpError::invalid_params(
...
pub async fn serve(store: DataStore, llm: Option<Arc<dyn LlmClient>>, port: u16) -> Result<()> {
