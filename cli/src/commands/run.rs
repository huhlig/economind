...
use economind_strategy::{
    run_strategy, CompositionMode, PipelineRunnerBuilder, StrategyConfig,
};
...
pub async fn execute(args: RunArgs, database_url: &str, duckdb_path: &str) -> anyhow::Result<()> {
    ...
    // Sync DuckDB from PostgreSQL before loading context.
    store
        .sync(args.lookback_days)
        .await
