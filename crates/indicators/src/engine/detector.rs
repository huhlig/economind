...
pub fn volume_sma(data: &[CandleEntry], period: usize) -> Vec<f32> {
    ...
    #[allow(clippy::needless_range_loop)]
    for i in period..data.len() {
        let sum: f32 = data[i - period..i].iter().map(|c| c.volume as f32).sum();
...
pub fn multi_timeframe_confluence(
    ...
    map.into_iter()
        ...
        .map(
            |(pattern, tfs): (PatternType, Vec<String>)| ConfluenceResult {
                pattern,
...
mod tests {
    ...
    fn atr_positive_once_window_fills() {
        ...
        assert!(
            result[14] > 0.0,
            "ATR should be positive after window fills"
        );
    }
    ...
    fn confluence_single_timeframe_no_result() {
        ...
        map.insert(
            "1d".to_string(),
            vec![make_pattern(PatternType::DoubleBottom)],
        );
    ...
    fn confluence_two_timeframes_detected() {
        ...
        map.insert(
            "1d".to_string(),
            vec![make_pattern(PatternType::HeadAndShoulders)],
        );
        map.insert(
            "1w".to_string(),
            vec![make_pattern(PatternType::HeadAndShoulders)],
        ...
        assert!(
            (results[0].confidence_boost - 0.30).abs() < 1e-5,
            "confidence_boost={}",
            results[0].confidence_boost
        );
    }
    ...
    fn confluence_three_timeframes_higher_boost() {
        ...
        assert_eq!(results.len(), 1);
        assert!(
            (results[0].confidence_boost - 0.45).abs() < 1e-5,
            "3 TF boost should be 0.45, got {}",
            results[0].confidence_boost
        );
    }
