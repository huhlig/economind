...
impl EnsembleRunner {
    ...
    pub fn new(stacks: Vec<(StrategyStack, f64)>, signal_threshold: f64) -> Self {
        assert!(!stacks.is_empty(), "EnsembleRunner requires at least one stack");
        let total: f64 = stacks.iter().map(|(_, w)| *w).sum();
        assert!(total > 0.0, "At least one stack weight must be > 0.0");

        let normalised: Vec<(StrategyStack, f64)> = stacks
            .into_iter()
            .map(|(s, w)| (s, w / total))
            .collect();
    ...
    pub async fn run(&self, ctx: &StrategyContext) -> Vec<TradeSignal> {
        ...
        for (symbol, contribs) in contributions {
            ...
            let weighted_score: f64 = contribs
                .iter()
                .map(|(w, t, _)| w * t.score)
                .sum::<f64>()
                / total_weight;
            ...
            let avg_shares =
                avg_decimal_weighted(contribs.iter().map(|(w, _, s)| (*w, s.shares)));
            let avg_notional =
            ...
            let contributing_stacks: Vec<&str> = self
                ...
                .iter()
                .filter(|(stack, _)| {
                    contribs.iter().any(|(_, t, _)| {
                        t.rationale.contains(&format!("[{}]", stack.name))
                    })
                })
...
impl WeightOptimizer {
        ...
        F: Fn(&[f64]) -> f64,
    {
        ...
        let mut f_vals: Vec<f64> = simplex
            .iter()
            .map(|v| score_fn(v))
            .collect();
        ...
        for _iter in 0..self.max_iterations {
            ...
            if f_reflected > f_vals[order[0]] {
                ...
                let contract_point = if use_reflected { &reflected } else { &simplex[worst_idx].clone() };
...
impl EnsembleRunnerBuilder {
    ...
    pub fn build(self) -> EnsembleRunner {
        EnsembleRunner::new(
            self.stacks,
            self.signal_threshold.unwrap_or(0.5),
        )
    }
}
