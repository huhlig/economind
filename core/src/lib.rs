    ...
    fn daily_candle_series_aggregates_correctly() {
        ...
        let entries = vec![
            DailyCandleEntry {
                ...
                open: dec("100"), high: dec("105"), low: dec("98"), close: dec("103"),
                volume: 1_000,
            },
            DailyCandleEntry {
                date: NaiveDate::from_ymd_opt(2024, 1, 2).unwrap(),
                open: dec("103"), high: dec("112"), low: dec("102"), close: dec("110"),
                volume: 2_000,
            },
        ];
    ...
    fn daily_candle_series_sorted_oldest_first() {
        ...
        let entries = vec![
            DailyCandleEntry {
                ...
                open: dec("110"), high: dec("115"), low: dec("108"), close: dec("112"),
                volume: 500,
            },
            DailyCandleEntry {
                date: NaiveDate::from_ymd_opt(2024, 1, 1).unwrap(),
        ...
        let series = DailyCandleSeries::from_iter(Symbol::new("TEST"), entries).unwrap();
        assert!(series.entries[0].date < series.entries[1].date, "Should be sorted oldest first");
    }
