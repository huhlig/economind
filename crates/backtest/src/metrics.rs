...
fn build_equity_curve(daily_values: &BTreeMap<NaiveDate, (Decimal, Decimal)>) -> Vec<EquityPoint> {
...
fn compute_daily_returns(daily_values: &BTreeMap<NaiveDate, (Decimal, Decimal)>) -> Vec<f64> {
...
fn compute_trade_metrics(trades: &[SimTrade]) -> TradeMetrics {
    ...
    let wins: Vec<&SimTrade> = trades
        .iter()
        .filter(|t| t.net_pnl > Decimal::ZERO)
        .collect();
    let losses: Vec<&SimTrade> = trades
        .iter()
        .filter(|t| t.net_pnl <= Decimal::ZERO)
        .collect();
    ...
    let avg_loss = if loss_count > 0 {
        losses.iter().map(|t| t.net_pnl.abs()).sum::<Decimal>() / Decimal::from(loss_count as u32)
    } else {
        Decimal::ZERO
    };
    ...
    let avg_hold_days = trades
        .iter()
        .map(|t| Decimal::from(t.hold_days))
        .sum::<Decimal>()
...
mod tests {
    ...
    fn equity_map(values: &[(i32, u32, u32, f64, f64)]) -> BTreeMap<NaiveDate, (Decimal, Decimal)> {
        values
            ...
            .map(|&(y, m, d, v, c)| {
                (
                    date(y, m, d),
                    (Decimal::from_f64(v).unwrap(), Decimal::from_f64(c).unwrap()),
                )
            })
            .collect()
    }
    ...
    fn equity_curve_monotone_rising_has_zero_drawdown() {
        ...
        for pt in &curve {
            assert_eq!(
                pt.drawdown,
                Decimal::ZERO,
                "rising series should have 0 drawdown"
            );
        }
    }
    ...
    fn equity_curve_drawdown_computed_correctly() {
        ...
        assert!(
            (dd - 10_000.0 / 110_000.0).abs() < 1e-6,
            "drawdown={:.6}",
            dd
        );
    }
    ...
    fn cagr_no_growth_returns_zero() {
        ...
        let cagr = compute_cagr(d("100000"), d("100000"), 252)
            .to_f64()
            .unwrap();
    ...
    fn cagr_doubles_in_one_year_is_100pct() {
        ...
        let cagr = compute_cagr(d("100000"), d("200000"), 252)
            .to_f64()
            .unwrap();
    ...
    fn cagr_positive_for_growing_portfolio() {
        let cagr = compute_cagr(d("100000"), d("120000"), 252)
            .to_f64()
            .unwrap();
        assert!(cagr > 0.0);
    }
    ...
    fn trade_metrics_expectancy_positive_edge() {
        ...
        assert!(
            exp > 0.0,
            "expectancy should be positive with 3:2 win ratio, got {}",
            exp
        );
    }
    ...
    fn trade_metrics_largest_win_and_loss() {
        let trades = vec![
            make_trade(200.0, 5),
            make_trade(50.0, 3), // wins
            make_trade(-150.0, 4),
            make_trade(-30.0, 2), // losses
        ];
        let m = compute_trade_metrics(&trades);
