...
impl YahooFinanceConnector {
    ...
    pub async fn fetch_metadata(&self, symbol: &Symbol) -> ProviderResult<Ticker> {
        ...
        let _sector = result.asset_profile.as_ref().and_then(|p| p.sector.clone());
...
impl DailyDataProvider for YahooFinanceConnector {
    async fn daily_bars(
        ...
        let result = root.chart.result.into_iter().next().ok_or_else(|| {
            ProviderError::OtherError(format!(
...
fn parse_chart_result(result: ChartResult) -> ProviderResult<Vec<DailyCandleEntry>> {
    ...
    let quote =
        result.indicators.quote.into_iter().next().ok_or_else(|| {
            ProviderError::OtherError("No quote indicators in chart result".into())
        })?;
