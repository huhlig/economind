...
impl PostgresStorage {
    ...
    async fn get_setting_decimal(&self, key: &str) -> StorageResult<Decimal> {
        let row = sqlx::query("SELECT value FROM system.settings WHERE key = $1")
            .bind(key)
...
impl StrategyStorage for PostgresStorage {
    ...
    async fn insert_strategy_config(&self, row: &StrategyConfigRow) -> StorageResult<()> {
        sqlx::query(
            ...
            INSERT INTO strategy.configs
                (id, name, description, composition, parameters, enabled,
                 auto_execute, execution_mode, version, created_at, updated_at)
    ...
    async fn query_strategy_signals(
        ...
        if let Some(s) = since {
            let dt: DateTime<Utc> =
                chrono::DateTime::from_naive_utc_and_offset(s.and_hms_opt(0, 0, 0).unwrap(), Utc);
            q = q.bind(dt);
        }
        q = q.bind(lim);
