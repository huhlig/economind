...
impl VotingRunner {
    ...
    pub fn new(stacks: Vec<StrategyStack>, quorum: f64) -> Self {
        assert!(
            !stacks.is_empty(),
            "VotingRunner requires at least one stack"
        );
        assert!(
    ...
    pub async fn run(&self, ctx: &StrategyContext) -> Vec<TradeSignal> {
        ...
        for (symbol, vote_list) in votes {
            ...
            let avg_timing_score = vote_list.iter().map(|(t, _)| t.score).sum::<f64>() / vote_count;
            ...
            let avg_fraction = average_decimal(vote_list.iter().map(|(_, s)| s.portfolio_fraction));
...
impl VotingRunnerBuilder {
    ...
    pub fn build(self) -> VotingRunner {
        VotingRunner::new(self.stacks, self.quorum.unwrap_or(0.5 + f64::EPSILON))
    }
}
...
mod tests {
    ...
    fn test_quorum_validation() {
        // Should not panic.
        let _ = VotingRunner::new(vec![StrategyStack::default()], 0.5);
    }
